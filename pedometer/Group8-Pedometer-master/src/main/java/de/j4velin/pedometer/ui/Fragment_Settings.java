...
public class Fragment_Settings extends PreferenceFragment implements OnPreferenceClickListener {
    ...
    final static int AVG_CALC_SETTINGS = 2;
    ...
    public boolean onPreferenceClick(final Preference preference) {
        ...
        switch (preference.getTitleRes()) {
            ...
            case R.string.avg_title:
                builder = new AlertDialog.Builder(getActivity());

                final NumberPicker picker = new NumberPicker(getActivity());
                picker.setMinValue(0);
                picker.setMaxValue(2);
                //FIXME: Make these values general for localization
                picker.setDisplayedValues( new String[] { "All Time Average Excluding Today", "Last 7 Days Average Excluding Today", "All Time Average Including Today" } );
                picker.setValue(prefs.getInt("avgSettings", 2) );

                builder.setView(picker);
                //FIXME: Make value general for localization
                builder.setTitle("Pick Average");

                builder.setPositiveButton(android.R.string.ok, new OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        picker.clearFocus();
                        prefs.edit().putInt("avgSettings", picker.getValue()).commit();
//                        preference.setSummary(getString(R.string.avg_summary, picker.getValue()));
                        dialog.dismiss();
                        getActivity().startService(new Intent(getActivity(), SensorListener.class)
                                .putExtra("updateNotificationState", true));
                    }
                ...
                Dialog dialogAvg = builder.create();
                dialogAvg.getWindow().setSoftInputMode(
                        WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE);
                dialogAvg.show();
